...
//      lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS " %s %s %s %s"), chrX, chrY, chrZ, chrP);
 //     lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS " Falta Agua"), chrX, chrY, chrZ, chrP);
//lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_XMIN_MSG), chrX, chrY, chrZ, chrP);
//      Add to the end of the setup() function:
//  enqueue_and_echo_commands_P(PSTR("M107 P0\nM117 PARADA general"));    ///DQ
  
  if (digitalRead(X_MIN_PIN) == 0){ 
  lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_XMIN_MSG));
    enqueue_and_echo_commands_P(PSTR("M107 P0\nM117 PARADA general"));    ///DQ
  }
  if (digitalRead(Y_MIN_PIN) == 0){
    lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_YMIN_MSG));
    enqueue_and_echo_commands_P(PSTR("M107 P0\nM117 PARADA general"));    ///DQ
  }
  if (digitalRead(Z_MIN_PIN) == 0){
   lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_ZMIN_MSG));
    enqueue_and_echo_commands_P(PSTR("M107 P0\nM117 PARADA general"));    ///DQ
  }
