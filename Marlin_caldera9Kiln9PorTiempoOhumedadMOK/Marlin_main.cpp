...
//--------------------------------------------------------------****************************************************************
//  Program:      MEDIR HUMEDAD MADERA POR VOLTAJE/OHMIOS DIVISOR

#include <Sensirion.h>
//definiciones para sensores especiales**************************************************
// como no es un I2C con direccion, se usan pines digitales independientes para cada sensor
// en este caso uso D5 para clock y otros D para los datos
// puertos redefinidos para ramps**************************************************
//const uint8_t SSdataPin  =  4; // Pin asignado en ramps pins
//const uint8_t SSclockPin =  5; // Pin asignado en ramps pins
//const uint8_t SSdataPin2 =  6; // Pin asignado en ramps pins

float SStemperature;
float SShumidity;
float SSdewpoint;

float SStemperature2;
float SShumidity2;
float SSdewpoint2;

//definiciones para ciclos de secado ( usando M665 para leer**************************************************float CicloPaso;
unsigned long CicloPasoDuracion = 0;    //Duracion especificada para el paso del ciclo en milisegundos
float CicloPaso = 0;
bool CicloPasoAlcanzado = 1;
float CicloT = 30;  //Temperatura de inicio
float CicloHR = 0;
float CicloHMad = 0;
bool CicloTNO = true;    //define si le falta pasar por chequeo de precalentamiento
bool NOFIN = true;    //NO se termino el ciclo, se activa cuendo ciclo cierra y apaga todo
char subbuff[19];
//const char *  ComandoActual = " Pre ciclo                        ";
Sensirion tempSensor = Sensirion(SSdataPin, SSclockPin);
Sensirion tempSensor2 = Sensirion(SSdataPin2, SSclockPin);

//DETERMINADOR HUMEDAD MADERA usando pin A5 de ramps
//number of analog samples to take per reading
#define Mad_PIN A5
#define MadNUM_SAMPLES 10
#define MadVoltRef 4.5    //voltaje referencia
#define MadResRef 10     //Resistencia divisor en MOhm 


//const uint8_t MadPin =  14;        // Pin asignado en ramps pins
//analogReference(DEFAULT);
int Madsum = 0;                    // sum of samples taken
unsigned char Madsample_count = 0; // current sample number
float Madvoltage = 0.0;            // calculated voltage
float MadMOhmMadera  = 0.0;        // resistencia calculada en Mega ohm
float Madhumedad = 0.0;            // humedad calculada =21.26*MOhmMadera^-0.111
float MadEspC  =   21.26;                 //Caoba =EspC*MOhmMadera^EspP Constante
float MadEspP  =   -0.111;                 //Caoba =EspC*MOhmMadera^EspP Potencia
//FIN definiciones para sensores especiales**************************************************

//****************************************Tiempos y variables para ciclo del KILN
unsigned long  TENTRE = 180000;   //Tiempo en miliseguntos (180000=3 minutos)entre ajustes del KILN Primero(tiempo de estabilización) 
unsigned long  TKiln = 0;
unsigned long  TKiln_ENTRE = 0;   //Tiempo pasado desde acciones de correccion
unsigned long  TEXTRAE = 0;       //Tiempo de extracción de humedad
unsigned long  TSPRAY = 0;        //Tiempo de sprayado
int FAN_VEL = 255;                //Velocidad de los fan (max)
...
inline void gcode_M17() {
  ...
  inline void gcode_M24() {
    ...
    #endif
    //DQ no hace pausa en ciclo      print_job_timer.start();
  }
  ...
  inline void gcode_M25() {
    card.pauseSDPrint();
 //   print_job_timer.pause();  //DQ no hace pausa en ciclo
...
inline void gcode_M31() {
  ...
  SERIAL_ECHO_START();
  SERIAL_ECHOLNPAIR("Tiempo Secado: ", buffer);
}
...
inline void gcode_M205() {
  ...
  /**
   * M665: Nuevo comando para leer variables de ciclo de secado **********************************************************************************************
   *
   *    A = MadEspC     Constante para curva humedad madera
   *    B = MadEspP     Potencia para curva humedad madera
   *    C = CicloPaso   Paso en el ciclo  Negativos es por tiempo, positivos por hum madera
   *    M = CicloHMad   Meta Humedad madera del paso
   *    T = CicloT      Temperatura de operacion del paso
   *    H = CicloHR     Humedad de operacion del paso
   */
inline void gcode_M665() {
    if (parser.seen('A')) MadEspC     = parser.value_float();
    if (parser.seen('B')) MadEspP     = parser.value_float();
    if (parser.seen('C')) CicloPaso   = parser.value_float();
    if (parser.seen('M')) CicloHMad   = parser.value_float();
    if (parser.seen('T')) CicloT      = parser.value_float();
    if (parser.seen('H')) CicloHR     = parser.value_float();
    if (CicloPaso == -1) CicloPasoDuracion = CicloHMad*3600000;
    if (CicloPaso < -1) CicloPasoDuracion = CicloPasoDuracion + CicloHMad*3600000;
    if (CicloPaso == 99) NOFIN = false;
//    if (parser.seen('Y')) delta_tower_angle_trim[B_AXIS] = parser.value_float();
//    if (parser.seen('Z')) delta_tower_angle_trim[C_AXIS] = parser.value_float();
//    recalc_delta_settings();
  }
// FIN* M665: Nuevo comando para leer variables de ciclo de secado ********************************************************************************************** 
/*
#if ENABLED(DELTA)
  /**
...
/**
   * M750: cheqKiln
   * Hace cambio de paso de SD si
      - Humedad madera inferior o igual a Humedad de madera meta
      o
      - Tiempo transcurrido igual o superior a tiempo meta
      o
      - Humedad post madera inferior a humedad meta del ciclo  NO IMPLEMENTADO
   *  
 */
...
inline void gcode_M750() {
  ...
  if (CicloPaso >0){      
      for ( Madsample_count; Madsample_count <= MadNUM_SAMPLES; Madsample_count++){     //take a number of analog samples and add them up
        Madsum += analogRead(5);      
        delay(1);
      }
      Madvoltage = ((float)Madsum / (float)MadNUM_SAMPLES * MadVoltRef) / 1024.0;
      MadMOhmMadera  = ((MadResRef * MadVoltRef / Madvoltage) - MadResRef) ; //Rw=((R1V1/V2)-R1),  R en MOhm
      Madhumedad =(MadEspC * pow(MadMOhmMadera, MadEspP));
      Madsample_count = 0;
      Madsum = 0;
  }
    tempSensor.measure(&SStemperature, &SShumidity, &SSdewpoint);
    tempSensor2.measure(&SStemperature2, &SShumidity2, &SSdewpoint2);

 TKiln_ENTRE = millis();
      
   if ( (CicloPaso >0 && Madhumedad>CicloHMad) || (CicloPaso <0 && CicloPasoDuracion > TKiln_ENTRE) ){     //si la madera no llego a su nivel de humedad del paso o al tiempo especificado
      enqueue_and_echo_commands_P(PSTR("M25"));    //Pause SD commands
          
      if (SStemperature2*1.1 <= CicloT){  //Verifica:  TempPost mas de 10% debajo?
        lcd_status_printf_P(0, PSTR(MSG_KILN_VENT_MAS)); //subir temp  ***********Ver como controlar
        if (FAN_VEL < 241){
          FAN_VEL = FAN_VEL * 1.05;
          digitalWrite (FAN_PIN, FAN_VEL) ;     //acelerar fans 5%
          SERIAL_ECHOLNPAIR ("Fan acelerado a ", FAN_VEL);        }
        else  lcd_status_printf_P(0, PSTR("Fan al maximo"));
      }
      else if (SStemperature > CicloT*1.05){  //Verifica:  TempPre mas de 5% encima?
        lcd_status_printf_P(0, PSTR(MSG_KILN_VENT_MEN)); //Decelerar ventiladores  10%*********Ver como controlar    
        if (FAN_VEL > 10){
          FAN_VEL = FAN_VEL * .95;
          digitalWrite (FAN_PIN, FAN_VEL) ;     //decelerar fans 5%      }
          SERIAL_ECHOLNPAIR ("Fan decelerado a ", FAN_VEL);        }
        else lcd_status_printf_P(0, PSTR("Fan al minimo"));
      }
      
      if (SShumidity < CicloHR*.9) {    //Verifica:  HumPre mas de 10% debajo? 
        lcd_status_printf_P(0, PSTR(MSG_KILN_SPRAY));
        TSPRAY = millis();
        digitalWrite (SPRAY_PIN, HIGH) ; //Humectar con spray 30seg
        SERIAL_ECHOLNPAIR ("Spraying ", SPRAY_PIN);
      }
      
      if (SShumidity > CicloHR*1.05 ){    //Verifica:  HumPre mas de 5% encima? 
        lcd_status_printf_P(0, PSTR(MSG_KILN_EXTRAE)); //Sacar aire humedo vents 30 seg      
        TEXTRAE = millis();
        digitalWrite (EXTRACT_PIN, 255) ; //Activate extraction
        SERIAL_ECHOLNPAIR ("Extrayendo humedad ", EXTRACT_PIN);
      }
      else if (SShumidity2 > CicloHR*1.1){  //Verifica:  HumPost mas de 10% encima?
        lcd_status_printf_P(0, PSTR(MSG_KILN_EXTRAE)); //Sacar aire humedo vents 30seg      
        TEXTRAE = millis();
        digitalWrite (EXTRACT_PIN, 255) ; //Activate extraction
        SERIAL_ECHOLNPAIR ("Extrayendo humedad ", EXTRACT_PIN);
      }
    delay (1000);
   // lcd_status_printf_P(0, subbuff);  
    }
    
    else if (CicloPaso > 0 && Madhumedad <= CicloHMad || CicloPaso < 0 && CicloPasoDuracion < TKiln_ENTRE){ //Si La madera llego a su nivel de humedad del paso o tiempo transcurrido, sigue con el proximo paso de SD
        SERIAL_ECHOLNPAIR ("Humedad/Tiempo alcanzada del paso: ", CicloPaso);
        stop();  //After a stop the machine may be resumed with M999
        enqueue_and_echo_commands_P(PSTR("M24"));  //resume ciclo, M24, madera llego a su nivel de humedad, o transcurrio el tiempo
    }
}
...
//      #if ENABLED(DELTA) || ENABLED(HANGPRINTER) para KILN***********************************************************************************
...
case 750: gcode_M750(); break;                            // M750: CheqKiln  DQ
...
void kill(const char* lcd_msg) {
  ...
  digitalWrite (ALARM_PIN, 255) ; //enviar señal a la alarma pin 34
  ...
  _delay_ms(2600); // Wait a short time (allows messages to get out before shutting down.
  cli(); // Stop interrupts
 ...
 */
void setup() {
pinMode(Mad_PIN, INPUT);    // sets the analog pin x as input
pinMode(34, OUTPUT);
pinMode(36, OUTPUT);
  #if ENABLED(MAX7219_DEBUG)
    max7219.init();
...
//Special preflight check para KILN ******************************************************************************************************************
     for ( Madsample_count; Madsample_count <= MadNUM_SAMPLES; Madsample_count++){     //take a number of analog samples and add them up
        Madsum += analogRead(Mad_PIN);      
     }
        // use 5.0 for a 5.0V ADC reference voltage
        // 5.015V, 4.75 is the calibrated reference voltage
      Madvoltage = ((float)Madsum / (float)MadNUM_SAMPLES * MadVoltRef) / 1024.0;
        // send voltage for display on Serial Monitor
        // voltage multiplied by 11 when using voltage divider that divides by 11. 11.132 is the calibrated voltage divide value
        //  Serial.print(voltage * 11.132);
        //En este caso el voltaje resultante sirve para calcular la conductividad (humedad) de la madera segun tablas preestablecidas
      MadMOhmMadera  = ((MadResRef * MadVoltRef / Madvoltage) - MadResRef) ; //Rw=((R1V1/V2)-R1),  R en MOhm
      Madhumedad =(MadEspC * pow(MadMOhmMadera, MadEspP));
  /*    SERIAL_ECHOPAIR(" V.:", Madvoltage);
        SERIAL_ECHOPAIR(" MOhm.:",MadMOhmMadera);
        SERIAL_ECHOLNPAIR(" %Humedad:",Madhumedad);
        SERIAL_ECHOPAIR(" Paso:", CicloPaso);
        SERIAL_ECHOPAIR(" MetaMad.:",CicloHMad);
 */     SERIAL_ECHOLNPAIR(" Meta%Humedad:",CicloHR);
        SERIAL_ECHOLNPAIR(" %Humedad:",Madhumedad);
//      lcd_status_printf_P(0, PSTR("Nueva medida humedad"));
    ...
    tempSensor.measure(&SStemperature, &SShumidity, &SSdewpoint);
    tempSensor2.measure(&SStemperature2, &SShumidity2, &SSdewpoint2);
  SERIAL_ECHOPAIR ("Sensor1: Temperatura: ", SStemperature);
  SERIAL_ECHOPAIR(" C, Humedad: ", SShumidity);
  SERIAL_ECHOPAIR(" %, Rocío: ", SSdewpoint);
  SERIAL_ECHOLN(" C");
  
  SERIAL_ECHOPAIR ("Sensor2: Temperatura: ", SStemperature2);
  SERIAL_ECHOPAIR(" C, Humedad: ", SShumidity2);
  SERIAL_ECHOPAIR(" %, Rocío: ", SSdewpoint2);
  SERIAL_ECHOLN(" C");
    

//Special precheck para KILN  sensores mecánicos
  if (digitalRead(X_MIN_PIN) == 1)  lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_XMIN_MSG));
//  if (digitalRead(X_MIN_PIN) == 1)  kill(PSTR(USE_XMIN_MSG));
  
  if (digitalRead(Y_MIN_PIN) == 1)  lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_YMIN_MSG));
  if (digitalRead(Y_MIN_PIN) == 1)  kill(PSTR(USE_YMIN_MSG));  //se activa
  
  if (digitalRead(Z_MIN_PIN) == 0)  lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS USE_ZMIN_MSG));
  if (digitalRead(Z_MIN_PIN) == 0)  kill(PSTR(USE_ZMIN_MSG));

//chequeo temperaturas sensores en rango razonable (min 20C) 
/*#define TEMP_MIN_ROUTINE(NR) \
 int   minttemp[NR] = HEATER_ ##NR## _MINTEMP; \
    if (analog2temp(minttemp_raw[NR], NR) < 20){ lcd_status_printf_P(0, PSTR("Falla termistor" NR))};
...
//chequeo temperaturas sensores en rango razonable (23 a 40 ambiente, 23 a 80 kiln) 
if (thermalManager.degBed()<23 || thermalManager.degBed()>40 || SStemperature<23 || SStemperature>90 || SStemperature2<23 || SStemperature2>90)
  kill(PSTR("Chequear Termistores"));

//fin chequeo temperaturas  
lcd_status_printf_P(0, PSTR("Kiln preparado"));  
  } 

//  const char *  Comando = "Lista";
//*************************************************************************************fin void setup
...
void checkMadera(){
 ...
 /*  SERIAL_ECHOPAIR(" V.:", Madvoltage);
        ...
        SERIAL_ECHOPAIR(" Paso:", CicloPaso);
        SERIAL_ECHOPAIR(" MetaMad.:",CicloHMad);
    SERIAL_ECHOPAIR(" Meta%Humedad:",CicloHMad);
            SERIAL_ECHOLNPAIR(" %Humedad:",Madhumedad);
         lcd_status_printf_P(0, PSTR("Nueva medida humedad"));
  */ 
      Madsample_count = 0;
 ...
 *
 *  - Chequea temperaturas y humedad
...
//endstops.event_handler();   //checar posibles problemas

    
  checkTempHum();

  if (CicloPaso >0){ checkMadera();}                //solo si ya se cargaron datos de la tarjeta definiendo parametros del ciclo como por mnejo de humedad de madera
  else { Madhumedad = 99.9;}
  
  if ((CicloPaso == 1 || CicloPaso == -1) && CicloTNO && millis() < TENTRE){           //Primer tiempo de estabilización de 3min, no hacer ajustes
    if (SStemperature < CicloT){ 
        lcd_status_printf_P(0, PSTR(MSG_HEATING));     //Bucle  hasta llegar a temperatura
        SERIAL_ECHOLNPAIR ("Sensor1: Temperatura baja: 3min espera ", SStemperature);
        }   
    else if ((CicloPaso == 1 && SStemperature >= CicloT) || (CicloPaso == -1 && CicloPasoDuracion < TKiln_ENTRE )) {
        lcd_status_printf_P(0, PSTR(MSG_KILN_TEMPOK));
        SERIAL_ECHOPAIR ("Temp.Inicio OK: ", SStemperature); 
        CicloTNO = false; 
        TENTRE = 30000;  //Tiempo de estabilización 30 segundos
        FAN_VEL = 128;          //regresa a velocidad media 
        digitalWrite (FAN_PIN, FAN_VEL) ;   //regresa a velocidad media 
    }
  }

//Repite chequeos cada cierto tiempo (TENTRE) hasta que la madera llegue a su punto de humedad o tiempo
    if (TENTRE < millis()-TKiln_ENTRE && NOFIN){   //Tiempo en miliseguntos (30000= .5 minutos)entre ajustes del KILN (tiempo de estabilización) 
    // if (10000 < millis()-TKiln_ENTRE && NOFIN ){     //reemplazar en operacion**********NOFIN es falso cuando terminamos todo el ciclo
      enqueue_and_echo_commands_P(PSTR("M750"));     //M750 es el comando para hacer los chequeos de humedad en la madera y ajustes correspondientes
                                                    // SOLO POR ACA AVANZA COMANDOS
   
      if ((TSPRAY + 30000) < millis()) digitalWrite (SPRAY_PIN, LOW) ;   //Para spray despues de 30segundos
      if ((TEXTRAE + 30000) < millis())digitalWrite (EXTRACT_PIN, LOW) ;   //Para extractor despues de 30segundos } //end (Madhumedad>CicloHMad) fin de paso
    //Chequeos de rutina
      if (digitalRead(X_MIN_PIN) == 1) { lcd_status_printf_P(0, PSTR(USE_XMIN_MSG));}  //cerrar la puerta
      delay (5000);
    lcd_status_printf_P(0, subbuff);
    SERIAL_ECHOLNPAIR("Que dice subbuff por aca: ", subbuff);
    }
...
//********************************fin chequeos KILN  
  ...
  if (commands_in_queue) {
    ...
    #if ENABLED(SDSUPPORT)
        char* command = command_queue[cmd_queue_index_r];    //dq......................................
      if (card.saving) {
...
SERIAL_ECHOLNPAIR("Que dice command: ", command);  //---------------------------------------------------------------------------------------------------------------
//no lcd_status_printf_P(0, PSTR(command));
//lcd_status_printf_P(0, command);
//     const char * Comando = command;

  if (parser.seen('X')) {
//    char *  ComandoActual = command;
 //   lcd_status_printf_P(0, command);
 //   lcd_status_printf_P(0, Comando);
//    lcd_status_printf_P(0, ComandoActual);

char *buff = command;
memcpy( subbuff, &buff[11], 15 );
subbuff[15] = '\0';
    lcd_status_printf_P(0, subbuff);
    SERIAL_ECHOLNPAIR("Que dice subbuff: ", subbuff);
  }
